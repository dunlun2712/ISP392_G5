...
//khai bao thanh phan xu ly database
    ...
    public List<Invoice> viewInvoices(String id) {
        List<Invoice> invoices = new ArrayList<>();
        String sql = "SELECT invoice_id, room_id, users_id, createdDate, description, paymentType, paymentDate, status, totalPayment FROM billService where users_id = ?";
        try {
            conn = connection; // Assuming connection is properly initialized elsewhere
            ps = conn.prepareStatement(sql);
            ps.setString(1, id);
    ...
    public void updateStatus(String invoiceId, String status) {
        ...
        try {
            String sql = "UPDATE billService SET status = ? WHERE invoice_id = ?";
            stm = connection.prepareStatement(sql);
